...
@socket.on('update', namespace='/')
def refresh(code):
    with open('sessions/' + code['password'], 'w') as update:
        update.write(code['data'])
    emit('update', open('sessions/' + code['password'], 'r').read()) #room=code['password']
